                      ...
                      struct io_info* info)
{
    ...
    uint64_t MaxWrite = atomic_fetch_uint64_t(&op_ctx->ctx_export->MaxWrite);
    uint64_t MaxOffsetWrite = atomic_fetch_uint64_t(&op_ctx->ctx_export->MaxOffsetWrite);
    ...
    if (size > MaxWrite)
    {
        ...
        if (info == NULL || info->io_content.what != NFS4_CONTENT_HOLE)
        {
            LogFullDebug(COMPONENT_NFS_V4,
    ...
    LogFullDebug(COMPONENT_NFS_V4, "offset = %" PRIu64 " length = %" PRIu64, offset, size);
