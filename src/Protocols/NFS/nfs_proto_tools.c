...
static fattr_xdr_result decode_acl(XDR* xdr, struct xdr_attrs_args* args)
{
    ...
    for (ace = acldata.aces; ace < acldata.aces + acldata.naces; ace++)
    {
         ...
         */
        if ((IS_FSAL_ACE_GROUP_ID(*ace) ? ace->who.gid : ace->who.uid) == -1)
        {
...
{
    return xdr_encode_nfs4_owner(xdr, args->attrs->owner) ? FATTR_XDR_SUCCESS : FATTR_XDR_FAILED;
}
...
{
    return xdr_encode_nfs4_group(xdr, args->attrs->group) ? FATTR_XDR_SUCCESS : FATTR_XDR_FAILED;
}
...
int nfs4_Fattr_Fill_Error(fattr4* Fattr, nfsstat4 rdattr_error)
{
    ...
    if (xdr_res == FATTR_XDR_SUCCESS)
    {
        ...
        if (LastOffset == 0)
        {
            /* no supported attrs so we can free */
