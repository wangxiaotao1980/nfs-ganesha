                     ...
                     struct nfs_resop4* resp)
{
    ...
    res_SETXATTR4->SETXATTR4res_u.resok4.sr_info.before = fsal_get_changeid4(data->current_obj);
    ...
    if (FSAL_IS_ERROR(fsal_status))
    {
        return res_SETXATTR4->status = nfs4_Errno_state(state_error_convert(fsal_status));
    }
    res_SETXATTR4->SETXATTR4res_u.resok4.sr_info.after = fsal_get_changeid4(data->current_obj);
    return res_SETXATTR4->status;
}
