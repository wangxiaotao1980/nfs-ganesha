...
static void release(struct fsal_obj_handle *obj_pub)
{
    ...
    struct handle *obj = 
        ({ 
            const typeof(((struct handle *) 0)->handle) * __mptr = (obj_pub); 
            (struct handle *)((char *) __mptr - __builtin_offsetof (struct handle, handle)); 
        });
...
{
    ops->release        = release;               //01
    ops->merge          = ceph_merge;            //02
    ops->lookup         = lookup;                //03
    ops->mkdir          = ceph_fsal_mkdir;       //04
    ops->mknode         = ceph_fsal_mknode;      //05
