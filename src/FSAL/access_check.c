...
#include "../include/FSAL/access_check.h"
...
int display_fsal_inherit_flags(struct display_buffer* dspbuf, fsal_ace_t* pace)
{
    ...
    return display_printf(dspbuf, "Inherit:%s%s%s%s",
                          ...
                          IS_FSAL_ACE_INHERIT_ONLY(*pace)
                              ? " inherit_only"
                              : "",
                          IS_FSAL_ACE_NO_PROPAGATE(*pace)
                              ? " no_propagate"
                              : "");
}
                     ...
                     fsal_ace_t* pace, bool is_dir)
{
    ...
    if(b_left > 0 && IS_FSAL_ACE_SPECIAL_ID(*pace))
        b_left =
                display_cat(dspbuf,
                            IS_FSAL_ACE_SPECIAL_OWNER(*pace)
                                ? " owner@"
                                : IS_FSAL_ACE_SPECIAL_GROUP(*pace)
                                      ? " group@"
                         ...
                         struct attrlist* p_object_attributes)
{
    ...
    mask =
            ((mode & S_IROTH) ? FSAL_R_OK : 0) | ((mode & S_IWOTH) ? FSAL_W_OK : 0) | ((mode & S_IXOTH) ? FSAL_X_OK : 0
            );
    LogFullDebug(COMPONENT_NFS_V4_ACL,
                               ...
                               bool owner_skip)
{
    ...
    {
        /* fall back to use mode to check access. */
