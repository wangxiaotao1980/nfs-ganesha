...
{
    ops->release        = mem_release;           //01
    ops->lookup         = mem_lookup;            //02
    ops->readdir        = mem_readdir;           //03
    ops->create         = mem_create;            //04
    ops->mkdir          = mem_mkdir;             //05
    ops->mknode         = mem_mknode;            //06
    ops->symlink        = mem_symlink;           //07
    ops->readlink       = mem_readlink;          //09
    ops->getattrs       = mem_getattrs;          //10
    ops->setattr2       = mem_setattr2;          //11
    ops->link           = mem_link;              //11
    ops->rename         = mem_rename;            //12
    ops->unlink         = mem_unlink;            //13
    ops->close          = mem_close;             //14
    ops->open2          = mem_open2;             //15
    ops->reopen2        = mem_reopen2;           //16
    ops->read2          = mem_read2;             //17
    ops->write2         = mem_write2;            //18
    ops->commit2        = mem_commit2;           //19
    ops->lock_op2       = mem_lock_op2;          //20
    ops->close2         = mem_close2;            //21
    ops->handle_to_wire = mem_handle_to_wire;    //22
    ops->handle_to_key  = mem_handle_to_key;     //23
}
