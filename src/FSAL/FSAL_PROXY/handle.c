...
static void* pxy_clientid_renewer(void* arg)
{
    ...
    while(!close_thread)
    {
        ...
        if(!sessionid_needed && pxy_rpc_renewer_wait(lease_time - 5))
        {
            ...
            rc = pxy_compoundv4_execute(__func__,
                                        NULL,
                                        1,
                                        &seq_arg,
                                        &res);
...
int pxy_init_rpc(const struct pxy_fsal_module* pm)
{
    ...
    rc = pthread_create(&pxy_recv_thread,
                        NULL,
                        pxy_rpc_recv,
                        (void *)&pm->special);
    if(rc)
    ...
    rc = pthread_create(&pxy_renewer_thread,
                        NULL,
                        pxy_clientid_renewer,
                        (void *)&pm->special);
    if(rc)
    {
        LogCrit(COMPONENT_FSAL,
...
#define FSAL_CREATE_NB_OP_ALLOC 5 /* SEQUENCE PUTFH OPEN GETFH GETATTR */
    ...
    st = pxy_do_close_4_1(op_ctx->creds,
                          &fhok->object,
                          &opok->stateid,
                          op_ctx->fsal_export);
    if(FSAL_IS_ERROR(st))
...
#define FSAL_GETATTR_NB_OP_ALLOC 3 /* SEQUENCE PUTFH GETATTR */
    ...
    if(nfs4_Fattr_To_FSAL_attr_savreqmask(attrs,
                                          &atok->obj_attributes,
                                          NULL) != NFS4_OK)
        return fsalstat(ERR_FSAL_INVAL, 0);
    ...
    *caller_perm_check = true;
    ...
    if(name || attrs_out || openflags & FSAL_O_TRUNC)
    {
        ...
        *caller_perm_check = false;
        ...
        st = fill_share_OPEN4args(&share_access,
                                  &share_deny,
                                  openflags);
        if(FSAL_IS_ERROR(st))
        {
        ...
        owner_len = strnlen(owner_val, sizeof(owner_val));
        /* inattrs and openhow */
        st = fill_openhow_OPEN4args(&openhow,
                                    inattrs,
                                    createmode,
                                    verifier,
                                    &setattr_needed);
    ...
    {
        rc = nfs4_Fattr_To_FSAL_attr(attrs_out,
                                     &atok->obj_attributes,
                                     NULL);
        if(rc != NFS4_OK)
            return nfsstat4_to_fsal(rc);
                              ...
                              struct attrlist* attrs_out)
{
    ...
    while(p)
    {
         ...
         */
        fsal_status_t st = pxy_lookup_impl(parent,
                                           exp_hdl,
                                           creds,
                                           p,
                                           &next,
