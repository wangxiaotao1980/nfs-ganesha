                   ...
                   unsigned int nb_dbop_prealloc)
{
    ...
    p_thr_info->dbop_pool = pool_basic_init("drop_pool", sizeof(db_op_item_t));
...
static int init_database_access(db_thread_info_t *p_thr_info)
{
    ...
    if (rc != 0) {
        ...
        {
            LogCrit(COMPONENT_FSAL, "ERROR: could not connect to SQLite3 database (file %s): %s", db_file, sqlite3_errmsg(p_thr_info->db_conn));
            sqlite3_close(p_thr_info->db_conn);
        } else 
        {
            LogCrit(COMPONENT_FSAL, "ERROR: could not connect to SQLite3 database (file %s): status=%d",
                db_file, rc);
        }
        return HANDLEMAP_DB_ERROR;
    }
