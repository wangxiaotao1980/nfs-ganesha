...
static void release(struct fsal_obj_handle* obj_hdl)
{
    ...
    {
        /* release RGW ref */
        (void)rgw_fh_rele(export->rgw_fs,
                          obj->rgw_fh,
                          0 /* flags */);
    }
...
static bool rgw_cb(const char* name, void* arg, uint64_t offset, uint32_t flags)
{
    ...
    status = lookup_int(rgw_cb_arg->dir_hdl,
                        name,
                        &obj,
                        &attrs,
                        RGW_LOOKUP_FLAG_RCB |
                             ...
                             bool* caller_perm_check)
{
    ...
    if(createmode == FSAL_NO_CREATE)
    {
        ...
        status = obj_hdl->obj_ops.open2(temp,
                                        state,
                                        openflags,
                                        FSAL_NO_CREATE,
                                        NULL,
