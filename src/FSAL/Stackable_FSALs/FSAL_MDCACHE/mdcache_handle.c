    ...
    struct state_t* state)
{
    ...
    status = mdcache_new_entry(export,
                               sub_handle,
                               attrs_in,
                               attrs_out,
                               new_directory,
                                    ...
                                    struct attrlist* attrs_out)
{
    ...
    status = mdcache_alloc_and_check_handle(export,
                                            sub_handle,
                                            new_obj,
                                            false,
                                            &attrs,
                                     ...
                                     bool* eod_met)
{
    ...
    if (*whence > 0)
    {
        ...
        aerr = mdcache_avl_lookup_k(directory,
                                    *whence,
    ...
    *eod_met = false;
         ...
         dirent_node = avltree_next(dirent_node))
    {
        ...
        if (status.major == ERR_FSAL_STALE)
        {
             ...
             */
            status = mdc_lookup_uncached(directory,
                                         dirent->name,
                                         &entry,
                                         NULL);
        }
                                         ...
                                         bool owner_skip)
{
    ...
    return fsal_test_access(obj_hdl,
                            access_type,
                            allowed,
                            denied,
                            owner_skip);
                                    ...
                                    const char* new_name)
{
    ...
    else
    {
        ...
        status = mdcache_dirent_add(mdc_newdir,
                                    new_name,
                                    mdc_obj,
                                    &invalidate);
                                      ...
                                      struct attrlist* attrs_out)
{
    ...
    status = mdcache_refresh_attrs(
        entry,
        (attrs_out->request_mask & ATTR_ACL) != 0,
        true);
