...
enum log_flag_index_t
{
    ...
    /*< Time field. */
    LF_EPOCH,
    /*< Server Epoch field (distinguishes server instance. */
    LF_CLIENTIP,
    /* <Client IP field. */
    LF_HOSTAME,
    /*< Server host name field. */
    LF_PROGNAME,
    /*< Ganesha program name field. */
    LF_PID,
    /*< Ganesha process identifier. */
    LF_THREAD_NAME,
    /*< Name of active thread logging message. */
    LF_FILE_NAME,
    /*< Source file name message occured in. */
    LF_LINE_NUM,
    /*< Source line number message occurred in. */
    LF_FUNCTION_NAME,
    /*< Function name message occurred in. */
    LF_COMPONENT,
    /*< Log component. */
    LF_LEVEL,
    /*< Log level. */
...
enum timedate_formats_t
{
    ...
    /*< No time/date. */
    TD_GANESHA,
    /*< Legacy Ganesha time and date format. */
    TD_LOCAL,
    /*< Use strftime local format for time/date. */
    TD_8601,
    /*< Use ISO 8601 time/date format. */
    TD_SYSLOG,
    /*< Use a typical syslog time/date format. */
    TD_SYSLOG_USEC,
    /*< Use a typical syslog time/date format that
                       also includes microseconds. */
    TD_USER,
    /* Use a user defined time/date format. */
};
...
char thread_name[16];
__thread
char log_buffer[LOG_BUFF_LEN + 1];
__thread
char* clientip = NULL;
...
void SetNameFunction(const char* nom)
{
    ...
    {
        LogWarn(COMPONENT_LOG, "Thread name %s too long truncated to %s", nom, thread_name);
    }
    clientip = NULL;
}
...
static void ArmSignal(int signal, void(*action)())
{
    ...
    {
        LogCrit(COMPONENT_LOG, "Failed to arm signal %d, error %d (%s)", signal, errno, strerror(errno));
    }
} /* ArmSignal */
...
void set_const_log_str(void)
{
    struct display_buffer dspbuf = {
        sizeof(const_log_str),
        const_log_str,
        const_log_str
    };
    struct display_buffer tdfbuf = {
        sizeof(date_time_fmt),
        date_time_fmt,
        date_time_fmt
    };
...
void init_logging(const char* log_path, const int debug_level)
{
    ...
    {
        fprintf(stderr,
                "Create error (%s) for STDERR log facility!",
                strerror(-rc));
        Fatal();
    }
    ...
    {
        fprintf(stderr,
                "Enable error (%s) for STDERR log facility!",
                strerror(-rc));
        Fatal();
    }
    rc = create_log_facility("STDOUT",
                             log_to_stream,
                             NIV_FULL_DEBUG,
                             LH_ALL,
                             stdout);
...
{
    DisplayLogComponentLevel(COMPONENT_MEM_ALLOC,
        (char *)file,
                             line,
                             (char *)function,
                             NIV_NULL,
...
void rpc_warnx(char* fmt, ...)
{
    ...
    display_log_component_level(COMPONENT_TIRPC,
                                "<no-file>",
                                0,
                                "rpc",
                                NIV_DEBUG,
                           ...
                           struct config_error_type* err_type)
{
    ...
    if (errcnt == 0)
    {
        if (logger->logfields != NULL)
        {
            LogEvent(COMPONENT_CONFIG, "Changing definition of log fields");
        ...
        {
            LogEvent(COMPONENT_CONFIG, "Switching to new component log levels");
...
{
    CONF_ITEM_TOKEN("Default_log_level", 
                    NB_LOG_LEVEL, 
                    log_levels, 
                    logger_config, 
                    default_level),
    CONF_ITEM_BLOCK("Facility", 
                    facility_params, 
                    facility_init, 
                    facility_commit, 
                    logger_config, 
                    facility_list),
    CONF_ITEM_BLOCK("Format", 
                    format_options,
                    format_init, 
                    format_commit,
                    logger_config,
